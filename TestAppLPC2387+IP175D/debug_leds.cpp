...
void update_debug_leds (const STP_BRIDGE* b)
{
	...
	if (STP_IsBridgeStarted(b) && STP_GetPortLearning(b, 0, cist_tree))
		FIO0CLR = (1 << 1);
	else
		FIO0SET = (1 << 1);

	if (STP_IsBridgeStarted(b) && STP_GetPortForwarding(b, 0, cist_tree))
		FIO0CLR = (1 << 4);
	else
		FIO0SET = (1 << 4);

	if (STP_IsBridgeStarted(b) && STP_GetPortOperEdge(b, 0))
		FIO0CLR = (1 << 5);
	else
		FIO0SET = (1 << 5);

	if (STP_IsBridgeStarted(b) && b->ports[0]->trees[cist_tree]->tcWhile)
		FIO0CLR = (1 << 27);
	else
		FIO0SET = (1 << 27);
	...
	if (STP_GetPortEnabled (b, 1))
		data &= ~(1 << 0);
	if (STP_IsBridgeStarted(b) && STP_GetPortLearning (b, 1, cist_tree))
		data &= ~(1 << 1);
	if (STP_IsBridgeStarted(b) && STP_GetPortForwarding (b, 1, cist_tree))
		data &= ~(1 << 2);
	if (STP_IsBridgeStarted(b) && STP_GetPortOperEdge (b, 1))
		data &= ~(1 << 3);
	if (STP_IsBridgeStarted(b) && b->ports[1]->trees[cist_tree]->tcWhile)
		data &= ~(1 << 4);
