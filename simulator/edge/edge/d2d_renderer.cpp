...
class d2d_renderer : public d2d_renderer_i
{
	event_manager _em;
...
public:
	d2d_renderer (win32_window_i& window, ID3D11DeviceContext* d3d_dc, IDWriteFactory* dwrite_factory, ID2D1Factory1* d2d_factory)
		: _window(window)
		, _d3d_dc(d3d_dc)
	...
	void create_d2d_dc()
	{
		...
		hr = _swap_chain->GetBuffer (0, IID_PPV_ARGS(&dxgiSurface)); rassert(SUCCEEDED(hr));
		...
		hr = _d2d_factory->CreateDxgiSurfaceRenderTarget (dxgiSurface, &props, &rt); rassert(SUCCEEDED(hr));

		rt->QueryInterface(&_d2d_dc); rassert(SUCCEEDED(hr));
	...
	void render_no_handlers(ID2D1DeviceContext* dc) const
	{
		...
		dc->SetTransform(edge::dpi_transform(_window.hwnd()));

		com_ptr<ID2D1SolidColorBrush> brush;
		static constexpr D2D1_COLOR_F red = { 1, 0, 0, 1 };
		dc->CreateSolidColorBrush (&red, nullptr, &brush);

		auto pw = edge::pixel_width(_window.hwnd());

		auto rect = inflate(client_rect(_window.hwnd()), -pw);
	...
	void process_wm_paint (HWND hwnd)
	{
		...
		*/
		if (_em.has_handlers<render_e>())
			render_e::invoker(_em).invoke(hwnd, _d2d_dc);
		else
		...
		*/
		...
		after_render_e::invoker(_em).invoke(_d2d_dc);

		rassert(_painting);
		_painting = false;
	}

	virtual win32_window_i& window() const override { return _window; }
	...
	virtual before_render_e::subscriber before_render() override { return before_render_e::subscriber(_em); }
	virtual render_e::subscriber render() override { return render_e::subscriber(_em); }
	virtual after_render_e::subscriber after_render() override { return after_render_e::subscriber(_em); }
	virtual dc_releasing_e::subscriber dc_releasing() override { return dc_releasing_e::subscriber(_em); }
	virtual dc_recreated_e::subscriber dc_recreated() override { return dc_recreated_e::subscriber(_em); }
	...
	void invalidate_caret()
	{
		...
		edge::invalidate(bounds, _window.hwnd());
	}
...
namespace edge
{
	...
	{
		return std::make_unique<d2d_renderer>(window, d3d_dc, dwrite_factory, d2d_factory);
	}
}
