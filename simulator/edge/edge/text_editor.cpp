...
namespace edge
{
	...
	public:
		text_editor (d2d_renderer_i* renderer, IDWriteTextFormat* format, theme_color_provider_i* tcp, const D2D1_RECT_F& rect, float lr_padding, std::string_view text)
			: _renderer(renderer)
			, _dwrite_factory(renderer->dwrite_factory())
			, _format(format)
			, _tcp(tcp)
			, _editorBounds(rect)
			, _lr_padding(lr_padding)
		{
			...
			_renderer->render().add_handler<&text_editor::on_render>(this);
			_tcp->theme_colors_changed().add_handler<&text_editor::on_theme_colors_changed>(this);
			invalidate();
		}

		~text_editor()
		{
			_tcp->theme_colors_changed().remove_handler<&text_editor::on_theme_colors_changed>(this);
			_renderer->render().remove_handler<&text_editor::on_render>(this);
			_renderer->hide_caret();
			invalidate();
		}

		void on_theme_colors_changed()
		{
			_renderer->window().invalidate();
			set_caret_screen_location_from_caret_pos();
		}
		...
		void set_caret_screen_location_from_caret_pos()
		{
			...
			auto dpi = edge::dpi(_renderer->window().hwnd());
			...
			_renderer->show_caret(b, _tcp->color_d2d(theme_color::text_editor_fore));
		}
		...
		virtual handled on_key_down (uint32_t virtualKey, modifier_key mks) override
		{
				...
				|| ((mks == modifier_key::control) && (virtualKey == VK_INSERT)))
			{
				if (_caret_pos != _selection_origin_pos)
				{
					...
					if (::OpenClipboard(_renderer->window().hwnd()))
					{
						if (::EmptyClipboard())
						{
		...
		void invalidate()
		{
			...
			edge::invalidate(bounds, _renderer->window().hwnd());
		}
		...
		{
			com_ptr<ID2D1SolidColorBrush> fill_brush = _tcp->make_brush(dc, theme_color::text_editor_back);
			...
			if (selectionEndIndex != selectionStartIndex)
			{
				...
				auto b = _tcp->make_brush(dc, theme_color::selected_back_focused);
				dc->FillRectangle (rect, b);
			}

			auto text_brush = _tcp->make_brush(dc, theme_color::text_editor_fore);
