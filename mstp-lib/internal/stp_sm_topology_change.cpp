...
struct TopologyChangeImpl : TopologyChange
{
	...
	static TopologyChange::State CheckConditions (const STP_BRIDGE* bridge, int givenPort, int givenTree, TopologyChange::State state)
	{
		assert (givenPort != -1);
		assert (givenTree != -1);
	...
	static void InitState (STP_BRIDGE* bridge, int givenPort, int givenTree, TopologyChange::State state, unsigned int timestamp)
	{
		assert (givenPort != -1);
		assert (givenTree != -1);
...
const SM_INFO<TopologyChange::State> TopologyChange::sm = {
	"TopologyChange",
	TopologyChangeImpl::GetStateName,
	TopologyChangeImpl::CheckConditions,
	TopologyChangeImpl::InitState
};
